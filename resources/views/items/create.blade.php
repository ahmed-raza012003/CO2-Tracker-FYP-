...
<div class="container mt-4">
    <h1 class="display-4 text-success mb-5 animate__animated animate__fadeInDown">Create New Item</h1>
    ...
    <form action="{{ route('items.store') }}" method="POST" class="shadow-lg p-5 rounded-lg" style="background-color: #C4E4C1;">
        @csrf

        <div class="mb-4">
            <label for="name" class="form-label text-success font-weight-bold">Name</label>
            <input type="text" name="name" class="form-control border-0" id="name" value="{{ old('name') }}" required>
        </div>

        <div class="mb-4">
            <label for="co2_value" class="form-label text-success font-weight-bold">CO2 Value</label>
            <input type="number" step="any" name="co2_value" class="form-control border-0" id="co2_value" value="{{ old('co2_value') }}" required>
        </div>
        ...
        <div class="text-center mt-4">
            <button type="submit" class="btn btn-success shadow">Create Item</button>
